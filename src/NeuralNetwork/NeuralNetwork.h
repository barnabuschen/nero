...
操作的实现形式：
		...
		这样来定义一个操作类型的nero：只要该nero  kind确定就由系统指定与一个实际完成操作的函数绑定
			...
			{
			nero_us32int msg;									/*记录该nero的种类，性质等信息
			nero_s32int x;      								recond  how many chindren			
...
#define  NeuronNode_EQUAL      151		//判断俩个obj指针中的对象是否是同一个，

//将  200  <  kind <  1000  作为kind 与操作实现函数自动映射的专用区间
// 问题，来了，想实现自动映射，一个函数指针数组内的函数指针要类型一致，就要有相同的参数和返回类型......
#define NeuronNode_OpKindAutoMappingStart    200
/*****************************************************************************/
#define NeuronNode_Addition  				 201	 	// 加 
#define NeuronNode_Subtraction 				 202		// 减 
#define NeuronNode_Multiplication  			 203		// 乘 
#define NeuronNode_Division   				 204		// 除





#define NeuronNode_OpKindAutoMappingEnd     1000
/*****************************************************************************/
