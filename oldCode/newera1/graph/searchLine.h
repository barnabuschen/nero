...
//////////下面是跟线条管理相关
//初始化一条线的表头，里面将动态分配一个内存
int InitialInitialPOSHead(PosList *Node);
//初始化一条线条集合管理表头，里面将动态分配一个内存
int InitialLineManNode(LineMan *Node);
...
//根据一些列的点的集合生成一个LineMan
//p是一个点的集合，表示了一系列的线条，每条线条用一个特殊的点隔开（x,y值都小于0的点），就是说PosCount实际上
//要大于真实的点数----注意第一条线的开头也要有个间隔点
//结果存于manAllLine中（manAllLine需要事先初始化好）
