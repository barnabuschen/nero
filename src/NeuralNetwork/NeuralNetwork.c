...
static nero_us32int tmpObiForTemporaryKind[tmpObiForTemporaryNUM];
...
void resetNeroConf()
{
	...
	neroConf.WantCreateObjKind=NeuronNode_ForNone;
	// neroConf.
...
/*			        printf("ifMakeUpWithTheseObjsInOrder: tmpObi=%x.\n",tmpObi);*/
		...
		// 这里有一个问题：
		// 是否要确认obj得数据个数要和objNum一样  ，即当且仅当obj由childred[]中得数据组成时return NeroYES;


		if (flag  == 1 &&  i == (objNum )   &&   objNum ==  nero_getObjDataNum(obj) )
		{
/*		        printf("i=%d. objNum=%d\n",i,objNum);*/
		        return NeroYES;
		}
...
/* 这个函数与前面俩个相比，它无关乎概念的类型，Obis里面可以是任何类型，只关乎层次上是否有共同的上层  */
		...
		*/
		...
		在findobj中返回这个找到得对象
		*/
nero_s32int   nero_IfHasObjFromMultiples3(NeuronObject *Obis[],nero_s32int objNum,nero_us32int searchKind,NeuronObject ** findobj)
{
	...
	if (Obis == NULL  || objNum < 2  ||  searchKind <=  NeuronNode_ForUndefined)
		return NeroError;
...
/*			printf("flag=%d.  obj=%x\n",flag,obj);*/
			/*判断这个对象是否包含所以子类型*/
			if (flag == 1   &&  nero_isBaseObj(obj) !=1  &&  searchKind ==  kind)
			{
				makeup=NeroNO;
				//现在有一个问题，就是这个函数可能在不同的线程中被调用
				//一旦在其他线程中被调用时，那么你就无法使用neroConf.WantCreateObjKind这个变量了

				// if(neroConf.WantCreateObjKind   !=   NeuronNode_ForNone   &&   kind ==   neroConf.WantCreateObjKind)
				{
...
// NeuronObject *Obis[],nero_s32int objNum,nero_s32int basekind,NeuronObject *godNero
	...
	nero_s32int cycleTimes,remainingNum,countInputDataNum,isOpObj,pointToCurrtObi,childKind,childOpCreate,childOpNum,newObjNUm;
...
// childOpFind
	...
	{
		//需要递归
		resObj=nero_IfHasOpObjFrMulti(Obis, objNum,newObiKind, godNero);
		if (resObj != NULL )
		{
			res= NeroYES;
		}		
...
/*	printf("createObjFromMultiples :newObiKind=%d.   childKind=%d\n",newObiKind,nero_GetNeroKind(Obis[0]));*/
	...
	/*生成新概念，并加入网络*/
	/*到这里意味着可以生成新的op操作了， 关于（forRe = 1 ：子数据都是操作类）或者不等于0 是否需要区别对待呢？
	forRe = 0 子数据都是数据类
		 按照常规就行
	forRe = 1 子数据都是操作类
		先创建子操作类的obj，生成一组新的 NeuronObject * Obis[], nero_s32int objNum
	*/
	...
	if(nero_msg_ok != res)
	{
		setNeroOperateFlag(newObi,1);
		#ifdef   nero_createOpByBaseKindInInSAP_DeBug_Msg
		printf("nero_createOpByBaseKindInInSAP：概念加入SAP网络失败id=%x Kind %d ,objNum=%d\n",newObi,newObiKind,objNum);
		// neroObjMsg_st.MsgId = MsgId_Log_PrintObjMsg;
	...
	else if (forRe == 1)//子数据都是操作类
	{
		//需要先生成子操作类的各个obj2222
		childOpNum = nero_getOpObjDataNum(newObiKind); 

		// res =NULL;
		newObjNUm =0;
		cycleTimes =0;
		childOpCreate =0;//找到的子操作数
		remainingNum = objNum;
		baseObj = nero_getBaseObjByKind(newObiKind,godNero);
		curFiber = baseObj->inputListHead;
		pointToCurrtObi =0;
		if (newObiKind > NeuronNode_ForComplexDerivative)
		{
			childOpNum = childOpNum -1;//实际的子操作数
			curFiber =curFiber->next;
			(chlidObjs)=(NeuronObject **)malloc(sizeof(NeuronObject *)* childOpNum);
		...
		while (remainingNum > 0 && childOpCreate < childOpNum)
		{
			...
			if (countInputDataNum <= remainingNum )
			{
				remainingNum = remainingNum - countInputDataNum;
				childKind = nero_GetNeroKind(childObj);
				tmpobj = nero_createOpByBaseKindInInSAP(childKind, &(Obis[pointToCurrtObi]),countInputDataNum,godNero);
				if (tmpobj == NULL)
				{
					printf("nero_createOpByBaseKindInInSAP：生成子操作失败1\n" );
					break;
				}
				else if( childOpCreate < childOpNum && tmpobj != NULL)
				{
					chlidObjs[childOpCreate] = tmpobj;
					childOpCreate++;
				}
				else
				{
					printf("nero_createOpByBaseKindInInSAP：生成子操作失败2\n" );
					break;
				}
			}
			curFiber = curFiber->next;
		...
		//
		//遍历成功，可以建立链接了
		if (remainingNum  == 0 &&  cycleTimes == childOpNum && childOpCreate == childOpNum )
		{
			for (i=0;i<childOpCreate;i++)
			{
				/*生成新概念的数据链表*/
				tmpFiber= addNerveFiber(newObi,NerveFiber_Input,Fiber_PointToData);
				tmpFiber->obj=chlidObjs[i];
				/*建立新概念已经子对象之间的关系*/
				PointingToObject(chlidObjs[i],newObi,Fiber_PointToUpperLayer);// put  newObi  in  Obis[i]  's   output list
				if (i>0)
				{
					PointingToObject(chlidObjs[i-1],chlidObjs[i],Fiber_PointToSameLayer);
				}
			}
		}
		else
...
nero_s32int nero_judgeNewObjKind(NeuronObject *Obis[],nero_s32int objNum)
{
	...
	*/
	if(neroConf.WantCreateObjKind  !=   NeuronNode_ForNone)
	{
			return neroConf.WantCreateObjKind;
	}
...
NeuronObject * nero_CreateObjInSAP(NeuronObject *Obis[],nero_s32int objNum,nero_s32int basekind,NeuronObject *godNero)
{
	...
	*/
	if(createNewBaseKindFlag  == 1  &&   neroConf.WantCreateObjKind  ==   NeuronNode_ForNone)
...
NeuronObject *  nero_createObjFromSingleObj(NeuronObject *childObi,nero_s32int upkind,NeuronObject * godNero)
{
	...
	while(tmpFiber  &&  tmpFiber->obj  != NULL )
	{
		if(   nero_GetNeroKind(tmpFiber->obj)  ==  upkind )
		{
			...
			if(    nero_isBaseObj(tmpFiber->obj)  !=  1)
			{
				if ( nero_getObjDataNum(tmpFiber->obj)   == 1)//only one child
				{
...
NeuronObject * nero_createObjFromMultiples(NeuronObject *Obis[],nero_s32int objNum)
{
	...
	*/
	...
	// 使用neroConf.WantCreateObjKind这个变量了,导致你必须只能在一个线程里面调用此函数，否则会出现变量数据不受控制的bug
	// nero_judgeNewObjKind  也是如此
	if(createNewBaseKindFlag  == 1  &&   neroConf.WantCreateObjKind  ==   NeuronNode_ForNone)
		newObiKind = neroConf.NewNeroClassID -1;
	else
	...
	// printf("create ObjFromMultiples :newObiKind=%d.   childKind=%d\n",newObiKind,nero_GetNeroKind(Obis[0]));
	/*生成新概念，并加入网络*/
	newObi= nero_createNeroObj (newObiKind);
	// printf("newObi=%x\n",newObi);
	res= nero_addNeroIntoNet( GodNero,newObi);
...
{
	const nero_us32int   strLenMax =  800;
	NeuronObject  *str[strLenMax];
...
// exit(0);
	...
	/*			printf("xx.\n");*/
		...
		if (tmp == NULL)
		{
		    printf("nero_addNeroByData:概念创建失败,strlenInData=%d,dataKind=%d\n",strlenInData,dataKind);
		}
...
// static nero_us32int tmpObiForTemporaryKind[tmpObiForTemporaryNUM];
	// NeuronObject  *str[strLenMax];
		...
		if(dataKind  <  NeuronNode_MinNewDerivativeClassId )
			break;
		tmp2=nero_getBaseObjByKind(dataKind,godNero);
		...
		neroObjMsgWithStr_st.Obi = NULL;
		sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default0  dataKind=%d,BaseObj=%x,godNero=%x ",dataKind,tmp2,godNero);
		msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
		#endif
				...
				neroObjMsg_st.MsgId = MsgId_Log_PrintObjMsg;
				neroObjMsg_st.fucId = 4;//Log_printAllNeroMsg
		...
		if(tmp2  !=  NULL)
		{
			tmpFiber = tmp2->inputListHead;
			kindAllTheSame = 1;
			if(tmpFiber ->next != NULL   &&  tmpFiber ->next->obj != NULL)
			{
				tmpFiber=tmpFiber ->next;
				for(i=0,tmp2 =NULL;tmpFiber != NULL  &&  tmpFiber ->obj != NULL;tmpFiber=tmpFiber->next ,i++)
				{
					// tmpObiForTemporary[i]=NULL;

					// getFiberPointToObjNum();

					// in case of ,  has  situation  of   Fiber_PointToMutiObj,
					tmpObiForTemporaryKind[i] = nero_GetNeroKind(tmpFiber->obj);
					// printf("nero_addNeroByData:默认处理---tmpObiForTemporaryKind[%d] =%d\n",i,tmpObiForTemporaryKind[i]);
					// tmpObiForTemporary[i]=tmpFiber->obj;
					if(i > 0  &&   tmpObiForTemporaryKind[i] !=  tmpObiForTemporaryKind[i-1])
						kindAllTheSame=0;
				...
				neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
				neroObjMsgWithStr_st.Obi = NULL;
				sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default1 ,kindAllTheSame=%d ",kindAllTheSame);
								msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
				...
				if(kindAllTheSame  == 1)
				{
					...
					neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
					neroObjMsgWithStr_st.Obi = NULL;
					if(tmpObiForTemporaryKind[0] !=  NeuronNode_ForChCharacter)
						sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default1 ,begin to check *p=%x ",*p);
					else
						sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default1 ,begin to check *p=%x ,%s",*p,p);
					msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
					...
					while(  (*p)  !=  0)
					{
						...
						// printf("nero_addNeroByData:默认处理---nero_IfHasNeuronObject,%x \n",tmp2);

					 	#ifdef Nero_DeBuging14_01_14
						neroObjMsgWithStr_st.MsgId = MsgId_Log_PrintObjMsgWithStr;
						neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
						neroObjMsgWithStr_st.Obi = NULL;
						sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default1 , tmpObiForTemporaryKind[0]=%d,tmp2=%x ",tmpObiForTemporaryKind[0],tmp2);
					   				msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);

					    #endif
						// nero_GetNeroKind();
						charLength=0;
						switch(tmpObiForTemporaryKind[0])
						{
							case NeuronNode_ForChCharacter:
						...
						if(charLength == 0  || tmp2 == NULL)
						{
						 	#ifdef Nero_DeBuging14_01_14
							neroObjMsgWithStr_st.MsgId = MsgId_Log_PrintObjMsgWithStr;
							neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
							neroObjMsgWithStr_st.Obi = NULL;
							sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default3 , error ,charLength=%d,tmp2=%x  ",charLength,tmp2);
						 				msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
					...
					if(childNun > 0  &&  hasKnowChar == 0 )
					{
						...
						neroObjMsgWithStr_st.Obi = NULL;
						if(tmp)
							sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default4 , createObj with childNun=%d,tmp=%x,kind=%d,datanum:%d ",childNun,tmp,nero_GetNeroKind(tmp),nero_getObjDataNum(tmp));
					   	else
							sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default4 , createObj with childNun=%d,tmp=%x",childNun,tmp);
					   	msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
				...
				else
				{
						...
						sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default2 , error kindAllTheSame=%d ",kindAllTheSame);
					   				msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
				...
				neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
				neroObjMsgWithStr_st.Obi = NULL;
				sprintf(neroObjMsgWithStr_st.str,"nero_addNeroByData:default0 , error ,nero_getBaseObjByKind  ");
								msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
...
NeuronObject * nero_IfHasNeuronObject(void *Data,nero_s32int dataKind,NeuronObject *GodNero)
{
	...
	/*			printf("xx.\n");*/
			...
			{
				case 1:
					words[i].first=*p;words[i].second=0;words[i].third=0;	p++;
					break;
				case 2:
					words[i].first=*p;words[i].second=*(p+1);words[i].third=0;p++;p++;
					break;
				case 3:
					words[i].first=*p;words[i].second=*(p+1);words[i].third=*(p+2);p++;p++;p++;
					break;
				default:
...
/*		wordP2=(ChUTF8  *)Data;*/
		wordP2=&(words[i]);/*实际上只是一个ChUTF8而非ChUTF8_结构的数据，但是不影响结果*/
		#ifdef Nero_DeBuging14_01_14_
		ttt22=Data;
	...
	case NeuronNode_ForChWord:
		...
		neroObjMsgWithStr_st.fucId = 1;
		neroObjMsgWithStr_st.Obi = NULL;
		sprintf(neroObjMsgWithStr_st.str,"在nero_IfHasNeuronObject:start find(ForChWord) :%s   child num : %d \n",Data,nero_getBaseObjChildenNum(NeuronNode_ForChWord,GodNero));
		msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
		#endif
	...
	case NeuronNode_ForOutputWord:
		// 1:假设数据只有一个字符
			...
			break;
	default:
			tmp =nero_IfHasNeuronObjectKindUnknow( Data ,dataKind, GodNero);
	...
	if(tmp == NULL)
	{
		...
		neroObjMsgWithStr_st.fucId = 1;
		neroObjMsgWithStr_st.Obi = NULL;
		sprintf(neroObjMsgWithStr_st.str,"在nero_IfHasNeuronObject : can not find obj for  kind:%d   tmp=%x",dataKind,tmp);
		msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
	}
	#endif
	return tmp;
...
NeuronObject * nero_IfHasNeuronObjectKindUnknow(void *Data,nero_s32int basekind ,NeuronObject *godNero)
{
	NeuronObject  *str[400];
	NerveFiber  *  curFiber;
	NerveFiber  *  childtmpFiber;

	ChUTF8  words[400];
	NeuronObject *tmpobj;
	NeuronObject *tmp2;
	NeuronObject *baseObj;
	nero_s32int strlenInData,flag,objNum,dataLen,hasDataFlag,childKind,charLength;
	ChUTF8  * wordP2;
	ChUTF8_  *wordP;
	nero_us8int  * ttt22;
	nero_us8int  * p;
	...
	curFiber=godNero->outputListHead;
	//find the  dataKind  baseobj
	while( curFiber != NULL  &&   nero_GetNeroKind(curFiber->obj) !=   basekind  )
	{
...
// 必须保证即使sys没有所查询数据得obj也能返回一个类型，除非没有满足得类型
nero_s32int nero_getObjKindByName(void *name,NeuronObject  * godNero)
{
	...
	if(tmp)
	{
		...
		while(tmpFiber)
		{
			...
			{
				// printf("所有对象kind为:%d,godNero=%x \n",nero_GetNeroKind(tmpFiber->obj),godNero);

				if(tmpFiber->obj->inputListHead->obj  ==  tmp)//tmp is  the  obj  of  name(kind = 62)
				{
			        ...
			        neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
			        neroObjMsgWithStr_st.Obi = NULL;
			        sprintf(neroObjMsgWithStr_st.str,"nero_getObjKindByName  4, findKind=%d \n",findKind);
			        msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
			    	#endif
	...
	else
	{
        ...
        neroObjMsgWithStr_st.fucId = 1;//打印某个具体obj得信息  Log_printSomeMsgForObj
        neroObjMsgWithStr_st.Obi = NULL;
        sprintf(neroObjMsgWithStr_st.str,"nero_getObjKindByName  3, error :can not  find kind of :%s \n",name);
        msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
    	#endif
...
/*
数据流Data中的数据表示一串metaDataKind kind obj的数据，
这个函数就是要根据这些数据找出这些obj，存于objsStore，返回找到的obj个数
metaDataKind = NeuronNode_ForChCharacter
*/
...
{
	nero_s32int findObjNum,lenOfData,charLength,strlenInData;

	NeuronObject  * str[TFFDataLength];
	// NerveFiber  *  curFiber;
	ChUTF8  words[TFFDataLength];
	// NeuronObject *tmp;
	// 	NeuronObject *tmp1;
	// NeuronObject *tmp2;
	// 	NeuronObject *tmpobj,* baseobj;
	nero_s32int i,allFindFlag,objNum;
	// ChUTF8  * wordP2;
	// ChUTF8_  *wordP;
	nero_s8int  * p,* StrEnd;
	// nero_us32int * dataPoint_;
	// nero_us8int  * ttt22;
	findObjNum =0;
	if(Data == NULL  ||   objsStore == NULL   ||  godNero == NULL     ||  metaDataKind != NeuronNode_ForChCharacter )
	{
    	printf("nero_getObjsByStr :   parameter error\n");
		return findObjNum;
	}
	lenOfData = strlen(Data);
	if(lenOfData >   TFFDataLength)
	{

    	printf("nero_getObjsByStr :  lenOfData >=   TFFDataLength\n");
		return findObjNum;
		...
		neroObjMsgWithStr_st.fucId = 1;
		neroObjMsgWithStr_st.Obi = NULL;
		sprintf(neroObjMsgWithStr_st.str,"nero_getObjsByStr:start find(ForChWord) :%s   child num : %d \n",Data,nero_getBaseObjChildenNum(NeuronNode_ForChWord,GodNero));
		msgsnd( Log_mq_id, &neroObjMsgWithStr_st, sizeof(neroObjMsgWithStr_st), 0);
		#endif
		allFindFlag=1;
		objNum=0;
	...
	/*			printf("xx.\n");*/
			...
			switch(charLength)
			{
				...
				case 3:
					...
					break;
				default:
					printf(" \n\n\n\n\nerror  :nero_getObjsByStr charLength =%d\n\n\n",charLength);
					return NULL;
					break;
			}
