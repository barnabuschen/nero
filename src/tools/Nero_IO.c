...
void IO_SaveSysIntoDatabase(NeuronObject  *godNero,NeroConf * conf)
{
	...
	if (godNero == NULL )
	{
		...
		printf("  IO_SaveSysIntoDatabase:GodNero=%x\n\n\n\n\n\n",GodNero);
		exit(0);
		return  ;
	}
...
数据化方案
				存储过程------------
1：考虑到nero和fiber不同的分配方式，动态分配的fiber地址总会比nero高
2：就nero来说，他的地址最低，可以作为基准，给他一个编号1或者0
			这样，所以的nero就可以作成一个list，进行存储，这个编号也是fiber-》obj的存储内容
			同时创建一个哈希表：保存一个nero的old地址和编号的数组
3：那怎么存储fiber的呢？
	考虑到fiber都动态的，所以可以忽略地址，只需要保存值就好了，就是说，将每个obj无论他是基类还是衍生类的obj都可以将
	inputListHead和outputListHead分别生成一个列表，列表名可以是nero的编号，列表的值是一个哈希表的名字，该哈希表示一个NerveFiber_对象
	（只需要存储obj，msg，time）

		生成filer时，为了能找到实际的nero，还需要在第二步中保存一个nero的新地址和编号的数组【在第二步骤中就需要这么列表了】-----
		假设有1亿个nero那么【100 000 000】 *  2 *  【一个指针的大小，一般是64位，就是8  B】  =  1.49GB
		------------考虑未来的大小，还是使用Redis Hashes  可以存储多达40亿个键-值对

step 1:  根据ActNero NeroPool[MaxNeroNum]; 创建一个哈希表1，保存所有nero的old地址和编号的数组
							编号为key{字符串}  ，valve为地址
					创建一个哈希表2，保存一个nero的数据
					方案1：把所有nero的数据存于一个哈希表2中--------key为nero编号，filed为各个数据 【暂时用这个方案】
					方案2：每个nero都设置一个哈希表
...
/******************************************/
	...
	for (;curFiber !=NULL;curFiber=curFiber->next)
	{
		...
		//save  base obj
		//除了基类其他obj都需要以链表或者集合方式保存，不然yi hou 一个个搜索太慢了

		inputListHead =  BaseObi->inputListHead;
		outputListHead =  BaseObi->outputListHead;
		// for(  ;  ;  )
		// {
...
*/
	...
	printf("IO_SaveSysIntoDatabase  over\n");
